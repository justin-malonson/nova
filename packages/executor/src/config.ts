...
const NETWORKS_ENV = (): string[] | undefined => {
  const networks = process.env["NOVA_NETWORKS"];
  if (networks) {
...
/**
 * str = baseGoerli => NOVA_BASE_GOERLI
 * str = goerli = NOVA_GOERLI
 * str = baseGoerli, suffix = ENTRYPOINTS => NOVA_BASE_GOERLI_ENTRYPOINTS
 */
function strToEnv(str: string, suffix = ""): string {
  const prefix = `NOVA_${str.replace(/([A-Z])+/, "_$1").toUpperCase()}`;
  if (suffix) {
