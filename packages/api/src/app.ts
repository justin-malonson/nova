...
import { NovaAPI } from "./modules/nova";
...
export interface LyfeblocNetworkBundlerOptions {
...
export class ApiApp {
  ...
  constructor(options: LyfeblocNetworkBundlerOptions) {
  ...
  private setupRouteFor(network: NetworkName): RouteHandler {
    ...
    const novaApi = new NovaAPI(relayer.eth, relayer.nova);
      ...
      if (result === undefined) {
        switch (method) {
          ...
          case BundlerRPCMethods.web3_clientVersion:
            ...
            break;
          case CustomRPCMethods.nova_validateUserOperation:
            result = await novaApi.validateUserOp({
              userOp: params[0],
              entryPoint: params[1],
            });
            break;
          case CustomRPCMethods.nova_getGasPrice:
            result = await novaApi.getGasPrice();
            break;
          default:
            throw new RpcError(
