...
6. run `./nova`
7. Nova will run for all chains available in `config.json`
...
2. edit `config.json`
3. `docker build -t lyfebloc-network/nova .`
4. `docker run --mount type=bind,source="$(pwd)"/config.json,target=/usr/app/config.json,readonly -dp 14337:14337 lyfebloc-network/nova start`
...
```yaml
{
  "networks": {
    "dev": { # network Id (check packages/types/src/networks/networks.ts)
      ...
      "relayer": "0xprivateKey", # relayer private key, can access from here or via environment variables (NOVA_MUMBAI_RELAYER | NOVA_DEV_RELAYER | etc.)
      "beneficiary": "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", # fee collector, avaiable via env var (NOVA_MUMBAI_BENEFICIARY | etc)
      "rpcEndpoint": "http://localhost:8545", # rpc provider, also available via env variable (NOVA_MUMBAI_RPC | etc)
      ...
      "gasPriceMarkup": 0, # adds % markup on reported gas price via nova_getGasPrice, 10000 = 100.00%, 500 = 5%
